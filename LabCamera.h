...
namespace Laborator
{
	...
	public:
		...
		void RotateFirstPerson_OX(float angle)
		{
			...
			glm::vec4 vector = glm::rotate(glm::mat4(1.0f), angle, right) * glm::vec4(forward, 1);
			forward = glm::normalize(forward + glm::vec3(vector));
		...
		void RotateFirstPerson_OY(float angle)
		{
			...
			forward = glm::normalize(forward + glm::vec3(newVector));

			newVector = glm::rotate(glm::mat4(1.0f), angle, glm::vec3(0, 1, 0)) * glm::vec4(right, 1);
			right = glm::normalize(right + glm::vec3(newVector));
		...
		{
			glm::vec4 vector2 = glm::rotate(glm::mat4(1.0f), angle, forward) * glm::vec4(right, 1);
			right = glm::normalize(right + glm::vec3(vector2));
			up = glm::cross(right, forward);
			// TODO
		...
		void RotateThirdPerson_OY(float angle, glm::vec3 center, float ballRadius)
		{
			// TODO
			float dist1 = position.x - center.x;
			float dist2 = position.z - center.z;
			//float dist3 = position.y - center.y;
			float distance = sqrt(dist1 * dist1 + dist2 * dist2);
